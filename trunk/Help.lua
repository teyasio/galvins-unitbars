...
For example you have a health, rage, and target health bar.  You want all three to be perfectly lined up with each other.  So if the rage bar is the bar you want first, then health second, and target health third.  First you move the rage bar in the exact spot you want the other two bars to be lined up with.  It doesn't matter where the other two bars are as long as the health bar is above the other two.  Horizontal position is not important.

Once that is done go to tools then align.  Pick the player power since this contains your rage if you're a warrior. Next pick the two other bars from the list below.  So you would check off player health and target health. Since we want our bars to be perfectly spaced apart vertically we'll enable vertical padding.  Then use the slider to pick how much padding you want.  Also since we want our bars to be lined up their right side we need to choose 'right' from the alignment drop down menu.  After that click align.  You should instantly see the tree bars all lined up vertically by their right side.

Once the align is clicked the tool going into a real time mode. What this allows you to do is change the alignment left or right or change the padding without having to click align each time.  But if you change the bars or primary bar to align with you'll need to click again .
